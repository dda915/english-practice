...
def _build_prompt(questions: list[Question]) -> str:
    ...
    lines += [
        ...
        "- ai_reading には英文本体とは別に、娘が書いた文型表記も含めて記録してください（例: 『They(S) were(V) very hungry(C) then(M).』のように）。",
        "- 文型の振り分けが間違っている場合は×にし、コメントでどこがどう違うかを具体的に指摘してください（例: 『hungryは補語(C)なのでOではなくCだよ』）。",
